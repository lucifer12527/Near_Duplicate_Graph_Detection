...
if (vf_order == '3') {
	...
	for (int j = 0; j != glen[0]; j++) {
		int pos = binary_search (path_graph[pdb[0][j]->hash_val], 0);
	...
	for (int i = 1; i != gdb_size; ) {
		switch(status[i]) {
			...
			for (int j = 0; j != glen[i]; j++) {
				int pos = binary_search (path_graph[pdb[i][j]->hash_val], i);
					...
					for (end = path_graph[pdb[i][j]->hash_val].rend(); ip != end; ++ip) {
						...
						if (free_end < *ip || *ip == 0) {
							...
							if (abs((int)gdb[i].total_num - (int)gdb[*ip].total_num) > tau) {
								free_end = *ip;
								// deletion
									pos = binary_search (path_graph[pdb[i][j]->hash_val], *ip);
