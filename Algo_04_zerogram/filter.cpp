...
bool zero_filtering( const unsigned p, const unsigned q){

			int sumextra;
			

					map<int,int> mymap1=fdb[p>q?q:p];
					map<int,int> mymap2=fdb[p>q?p:q];

					sumextra=0;
					map<int,int>::iterator iter1=mymap1.begin();
					for(map<int,int>::iterator iter2 = mymap2.begin(); iter2 != mymap2.end(); iter2++ ) {
       					 
       					 iter1=mymap1.find((*iter2).first);
       					 if(iter1==mymap1.end()){
       					 	sumextra=sumextra+( (*iter2).second );
       					 	continue;
       					 }else{
       					 	if((*iter2).second>(*iter1).second){
       					 		sumextra=sumextra+( (*iter2).second - (*iter1).second );
       					 	}
       					 }														
   					}
   					if(sumextra<=tau){	
