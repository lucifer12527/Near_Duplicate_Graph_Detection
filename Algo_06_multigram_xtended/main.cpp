...
int tau = 3, over_tau = 4, qs, path_qs, under_qs, my;
unsigned max_vnum;
bool* done;
double timer = 0;
...
vector<fmap >* res;	    // For storing the pairs 
unsigned* glen;                 // prefix length
fmap path_freq;
gmap path_graph;
vector<map<int,int> > fdb;
...
void parse_data(char* file) {
	...
	done = init_bool_array(true, LAG_SIZE);          // here we are initializig a boolean arry of size 1000 (default lagsize)
	set<int> edge_label;                             // and we are initializing such that all the members are true
	char tag;                                        // gdb is a vector of graphs.gdb.back always points to last element of vector
	fin >> tag;                                      // we keep on increasing size of gdb as we want to read each new graph
	while (!fin.eof()) {
		gdb.resize(gdb.size() + 1);
		tag = gdb.back().read(fin, tag, edge_label); // here we just a name of variable and at receiving we can decide to take by ref or value
	...
	gdb_size = gdb.size();						// first criteria of sort |G|, then |V| then |E| if all same  then left graph<right graph
	max_vnum = gdb.back().vertex_num;           //max_vnum is storing number of vertices in the latest graph
...
void init_mem()
{
	...
	res = new vector<fmap>[total_size + 1];
}

void clear () {
	int total_size = gdb.size();
	...
	memset(status, 0, sizeof(int)*total_size);
	memset(glen, 0, sizeof(unsigned)*total_size);
	path_freq.clear();	
	path_graph.clear();
	uid.clear();	
