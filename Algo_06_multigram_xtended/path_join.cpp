...
// Creating Data structure for 0-gram
void create_fdb(){
	for(int i=0;i<gdb.size();i++){	
		map<int,int> mymap;
		map<int,int>::iterator it;

		for(int j=0;j<gdb[i].vertex_num;j++){			
			it = mymap.find(gdb[i][j].vlabel);
			if(it == mymap.end())
				mymap.insert (it, std::pair<int,int>(gdb[i][j].vlabel,1));
			else
				(it->second)++;
		}
  		fdb.push_back(mymap);
...
{
	unsigned* tested = new unsigned[gdb_size], test_cnt = -1;
	int  free_end = 0;
	int* backup = new int[gdb_size], ans = 0, dist_cand = 0, cand = 0, temp = 0;
	vector<unsigned>::reverse_iterator ip, end, is;
...
if (vf_order == '3') {
	...
	for (int i = 0; i != gdb_size; ) {
		...
		switch(status[i]) {
			...
			for (int j = 0; j != glen[i]; j++) {
				...
				else if (j > 0 && pdb[i][j]->hash_val == pdb[i][j-1]->hash_val)
					continue;
				// Case where hash value is already present and graph i is not
				else {
					//ip = path_graph[pdb[i][j]->hash_val].rbegin();
					int p = path_graph[pdb[i][j]->hash_val].size();
					// Loop to through each element of that vector from back side
					//for (end = path_graph[pdb[i][j]->hash_val].rend(); ip != end; ++ip) {
					for (p--; p >= 0; p--) {
						int ph = path_graph[pdb[i][j]->hash_val][p];
						if (status[ph] == -1) 
							continue;
						tested[++ test_cnt] = ph;
						backup[test_cnt] = status[ph];             // backup array to store status array value
						status[ph] = -1;                            // Updating the status array
						if (free_end <= ph) {
							// size condition
							if (abs((int)gdb[i].total_num - (int)gdb[ph].total_num) > tau) {
								free_end = ph;
								// Deletion
								//int pos = binary_search (path_graph[pdb[i][j]->hash_val], *ip);
								if (p > 10) {	
									beg = path_graph[pdb[i][j]->hash_val].begin();		
									path_graph[pdb[i][j]->hash_val].erase(beg, beg + p + 1);
								}
								break;
							}
							// Filters
							if (size_filtering(i, ph) /*&& zero_filtering(*ip,i)*/) {
								++ cand;
								if (label_filtering(i, ph)) {
									pri = content_filtering(i, ph);
									if (pri != NULL) {
										++ dist_cand;
										// Creating the data structure for q = 3
										if (my == 3) {
											res[i][0][ph] = 1;
										}
										else if (my == 4 && res[i][0][ph] == 1) {
											temp++;
											// GED Computation
	...
	for (unsigned i = 0, m = uid.size(); i != m; ++ i) {
		for (unsigned j = i + 1, n = uid.size(); j != n; ++ j) {
			...
			if (size_filtering(uid[j], uid[i]) /*&& zero_filtering(uid[i],uid[j])*/) {
				++ cand;
				if (label_filtering(uid[j], uid[i])) {
					if (gdb[uid[j]].path_num == 0 && gdb[uid[i]].path_num == 0) {
						...
						else if (my == 4 && res[uid[i]][0][uid[j]] == 1) {
							temp++;	
							// GED Computation				
							compute_rud_dist(pri, ans, filter_only);
						}
						...
						if (pri != NULL) {
							...
							else if (my == 4 && res[uid[j]][0][uid[i]] == 1) {
								temp++;
								// GED Computation
								compute_rud_dist(pri, ans, filter_only);
							}
		...
		timer = (end1 - begin);
	#endif
	cout<<"No. of Pairs Whose GED is calculated : "<<temp<<endl;
	ECHO_JOIN
